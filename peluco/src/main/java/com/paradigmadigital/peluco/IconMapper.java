...
public class IconMapper {
    ...
    public int map(String key, long sunrise, long sunset) {
        if (key == null) {
            return 0;
        }
        ...
        return ICON_MAP.get("nt_" + key);
    }

    private boolean isDayLight(long sunrise, long sunset) {
        Time time = new Time();
        time.setToNow();
        long now = TimeUnit.HOURS.toMillis(time.hour) + TimeUnit.MINUTES.toMillis(time.minute);
